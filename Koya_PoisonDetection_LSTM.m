...
layers = [
    featureInputLayer(128,'Normalization', 'zscore');
    lstmLayer(96*2,'OutputMode','sequence')
    dropoutLayer(0.1493)
    lstmLayer(96,"Name","lstm","OutputMode","last")
    dropoutLayer(0.1493,"Name","dropout")
...
options = trainingOptions('rmsprop', ...
    ...
    'InitialLearnRate',0.0101, ...
    'LearnRateDropPeriod',2, ...
    'MaxEpochs',50, ...
    'L2Regularization',1.8495e-5, ...
    'Verbose',false);
[Avg_std_ACC, Avg_std_AUC, Avg_std_TPR,Avg_Std_TNR,Avg_std_F1Score, confuMatrices,models]=Binary_SVM_optimised(CleanWater,ToxicWater,layers,options);

toc


%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Avg_std_ACC, Avg_std_AUC, Avg_std_TPR,Avg_Std_TNR,Avg_std_F1Score, confuMatrices,models]=Binary_SVM_optimised(CleanWater,ToxicWater,layers,options)
...
% Training LSTM model and optimizing all of its hyperparameters.
LSTMModel1 = trainNetwork(training_data1(:,1:end-1),categorical(training_data1(:,end)),layers,options);
LSTMModel2 = trainNetwork(training_data2(:,1:end-1),categorical(training_data2(:,end)),layers,options);
LSTMModel3 = trainNetwork(training_data3(:,1:end-1),categorical(training_data3(:,end)),layers,options);
LSTMModel4 = trainNetwork(training_data4(:,1:end-1),categorical(training_data4(:,end)),layers,options);
LSTMModel5 = trainNetwork(training_data5(:,1:end-1),categorical(training_data5(:,end)),layers,options);

%%% Prediction stage for each of the models
[fold1_label,fold1_score] = classify(LSTMModel1,testing_data1);
[fold2_label,fold2_score] = classify(LSTMModel2,testing_data2);
[fold3_label,fold3_score] = classify(LSTMModel3,testing_data3);
[fold4_label,fold4_score] = classify(LSTMModel4,testing_data4);
[fold5_label,fold5_score] = classify(LSTMModel5,testing_data5);
...
models={LSTMModel1,LSTMModel2,LSTMModel3,LSTMModel4,LSTMModel5};
i
end
